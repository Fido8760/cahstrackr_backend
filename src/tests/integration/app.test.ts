...
import server, { conncetDB } from '../../server'
...
describe('Authentication - Create account', () => {
    ...
    it('should return 201 status code when the password and the email is correct', async () => {
        const response = await request(server)
                                .post('/api/auth/create-account')
                                .send({
                                    "name": "Fidel",
...
describe('Authentication - Account confirmation with token', () => {
    ...
    it('Should display error if token doesent exists', async () => {
        const response = await request(server)
                                    .post('/api/auth/confirm-account')
                                    .send({
                                        token: "123456"
    ...
    it('Confirm accont with a valid token', async () => {
...
describe('Authentication - Login', () => {
    ...
    it('should return 400 error if the user not found', async () => {
        const response = await request(server) 
                                .post('/api/auth/login')
                                .send({
                                    "password": "password",
    ...
    it('should return 403 error if the user is not confirmed', async () => {
        ...
        expect(response.body).toHaveProperty('error')
        expect(response.body.error).toBe('Usuario no confirmado')
    ...
    it('should return a jwt', async () => {
        ...
        expect(checkPassword).toHaveBeenCalledTimes(1)
        expect(checkPassword).toHaveBeenCalledWith('passwordCorrect', 'hashedPassword')
...
describe('Get /api/budgets', () => {
    ...
    beforeAll(() => {
        jest.restoreAllMocks()// restaurar las funciones de los jest.spy a su implementación orginal
    })
    ...
    it('should reject unauthenticated acces to budgets without a valid jwt', async () => {
        const response = await request(server)
                                    .get('/api/budgets')
...
describe('Put /api/budgets/:id', () => {
    ...
    it('display validation errors if the form is empty', async () => {
        const response = await request(server)
                                    .put('/api/budgets/1')
                                    .auth(jwt, {type: 'bearer'})
                                    .send({})
...
describe('Delete /api/budgets/:id', () => {
    ...
    it('should return 404 not found when budget doesne t exosts ', async () => {
        const response = await request(server)
