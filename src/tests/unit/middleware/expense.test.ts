...
describe('Expenses midddleware - validateExpenseExists', () => {

    beforeEach(() => {
        (Expense.findByPk as jest.Mock).mockImplementation( (id) => {
            const expense = expenses.filter(e => e.id === id)[0]
            return Promise.resolve(expense)
        })
    })

    it('Should handle a none existent budget', async () => {
    ...
    it('Should handle interneal server error', async () => {
